...
<script setup>
const citations = [
  "Aujourd'hui, il n'y a plus grand-chose de sûr...",
  "Est-ce que ce sont des informations vérifiées, ces milliers de vulnérabilités ? Ou juste de la comm parce que plus personne ne va vérifier, maintenant qu'il suffit de demander à un chatbot ?",
  ...
  "Et à titre personnel cette situation m'inquiète d'autant plus quand je vois que de grands chercheurs ou chercheuses de l'IA préfèrent disserter sur des conséquences [...] alors que des gens subissent déjà les conséquences matérielles bien concrètes de cette technologie",
  ...
  "le gain de temps (et donc de coûts) est censé être un des principaux objectifs recherchés par l'utilisation de l'IAg — ce serait ballot qu'au final ça induise plus de temps passé sur des tâches beaucoup moins intéressantes pour un•e développeur•euse",
